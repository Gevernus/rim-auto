...
const DirectLeasingTab = () => {
	...
	const buildDocumentUrl = (doc) => {
		...
		let path = doc.path || '';
		if (!path) return '';
		path = path.replace(/^documents\//, '/static/direct_leasing_docs/');
	...
	const loadApplications = async () => {
		...
		try {
			...
			const response = await applicationsApi.getDirectLeasingApplications(params);
	...
	const onUpdateStatus = async (applicationId, newStatus) => {
		try {
			await applicationsApi.updateDirectLeasingStatus(applicationId, newStatus);
	...
	const onDelete = async (applicationId) => {
		...
		try {
			await applicationsApi.deleteDirectLeasingApplication(applicationId);
			await loadApplications();
		} catch (error) {
